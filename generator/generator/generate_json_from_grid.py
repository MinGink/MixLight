        ...
        for j in range(columnNumber):
            for k in range(4):
                if roads[i][j][k] is not None and shouldDraw(roads[i][j][k]):
                    ...
                    roads[i][j][k].pop("toi")
                    roads[i][j][k].pop("toj")
                    roads[i][j][k]["startIntersection"] = checkIntersection(roads[i][j][k]["startIntersection"],
                                                                            isTruelyInside)
                    roads[i][j][k]["endIntersection"] = checkIntersection(roads[i][j][k]["endIntersection"],
                                                                          isTruelyInside)
